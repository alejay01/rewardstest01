# Source Notes

Last reviewed: 2026-05-06

These sources were checked while preparing the planning package. Pricing and provider rules can change, so verify again before purchase or launch.

## Hosting And cPanel

- HostGator PHP version guidance: https://www.hostgator.com/help/article/what-version-of-php-are-you-using
- HostGator cron jobs: https://www.hostgator.com/help/article/how-do-i-create-and-delete-a-cron-job
- cPanel MySQL Databases documentation: https://docs.cpanel.net/cpanel/databases/mysql-databases/
- HostGator shared hosting overview: https://www.hostgator.com/web-hosting.html

## SMS

- Telnyx is selected for the pilot SMS provider: https://telnyx.com/
- Telnyx SMS API product page: https://telnyx.com/products/sms-api
- Telnyx Messaging API docs: https://developers.telnyx.com/docs/messaging
- Telnyx receive message webhooks: https://developers.telnyx.com/docs/messaging/messages/receive-message
- Telnyx messaging webhook verification and delivery events: https://developers.telnyx.com/docs/messaging/messages/receiving-webhooks
- Telnyx 10DLC registration guide: https://support.telnyx.com/en/articles/6325731-register-for-10dlc-messaging
- Telnyx 10DLC campaign checklist: https://support.telnyx.com/en/articles/9038141-messaging-10dlc-campaign-checklist
- Twilio A2P 10DLC campaign approval requirements: https://help.twilio.com/hc/en-us/articles/11847054539547-A2P-10DLC-Campaign-Approval-Requirements
- Telnyx messaging pricing: https://telnyx.com/pricing/messaging/
- Telnyx 10DLC FAQ: https://support.telnyx.com/en/articles/3679260-frequently-asked-questions-about-10dlc
- Telnyx 10DLC developer docs: https://developers.telnyx.com/docs/messaging/10dlc
- Twilio U.S. SMS pricing: https://www.twilio.com/en-us/sms/pricing/usa
- Twilio A2P 10DLC compliance: https://www.twilio.com/docs/messaging/compliance/a2p-10dlc
- Twilio A2P quickstart: https://www.twilio.com/docs/messaging/compliance/a2p-10dlc/quickstart

## Email And Push

- OneSignal pricing: https://onesignal.com/pricing
- Brevo pricing plan article: https://help.brevo.com/hc/en-us/articles/208589409-About-Brevo-s-pricing-plans
- Firebase pricing, including Cloud Messaging: https://firebase.google.com/pricing

## PWA Support

- MDN PWA overview: https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps
- MDN installable PWA guide: https://developer.mozilla.org/en-US/docs/Web/Progressive_web_apps/Guides/Making_PWAs_installable
- web.dev PWA guide: https://web.dev/learn/pwa/progressive-web-apps
- WebKit Safari 16.4 features, including iOS/iPadOS Web Push: https://webkit.org/blog/13966/webkit-features-in-safari-16-4/
- Apple web push documentation: https://developer.apple.com/documentation/usernotifications/sending-web-push-notifications-in-web-apps-and-browsers
- Microsoft Edge PWA documentation: https://learn.microsoft.com/en-us/microsoft-edge/progressive-web-apps/

## POS And Loyalty Integrations

- Square Loyalty API overview: https://developer.squareup.com/docs/loyalty-api/overview
- Square Loyalty API reference: https://developer.squareup.com/reference/square/loyalty
- Square Loyalty product page: https://squareup.com/us/en/software/loyalty
- Toast third-party loyalty for Online Ordering Pro: https://support.toasttab.com/article/Toast-Online-Ordering-Pro-Third-Party-Loyalty
- Toast guest-facing loyalty display: https://support.toasttab.com/en/article/Sign-In-Redeem-Rewards-With-a-Guest-Facing-Display-Loyalty-API

## Compliance

- FTC CAN-SPAM compliance guide: https://www.ftc.gov/business-guidance/resources/can-spam-act-compliance-guide-business
- FCC TCPA consent discussion: https://docs.fcc.gov/public/attachments/DOC-408396A1.pdf
- CTIA messaging principles and best practices: https://api.ctia.org/docs/default-source/default-document-library/170119-ctia-messaging-principles-and-best-practices.pdf
